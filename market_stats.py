...
def plot_distribution(sequence, x: int, y: int):
    ...
    fig, axs = plt.subplots(num_rows, num_cols, figsize=(30, 30)) # set size
...
plot_distribution(mcap_sequence, (-2, 14), (0, 700))
plot_distribution(tvol_sequence, (-4, 12), (0, 1000))
...
def valuation(coin_of_interest):
...
def plot_result(df, name):
    ...
    # plot MarketCap and TradingVol versus Date
...
# %%
result = valuation("JESUS")
plot_result(result, 'JESUS')

# %%
coins = df['Symbol'].to_list()

for coin in tqdm(coins):
    result = valuation(coin)
    plot_result(result, coin.upper())