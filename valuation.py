...
df_YF = pd.read_csv("https://query1.finance.yahoo.com/v7/finance/download/BTC-USD?period1=1410825600&period2=1707264000&interval=1d&events=history&includeAdjustedClose=true")

df_CM['time'] = pd.to_datetime(df_CM['time'])
df_YF['Date'] = pd.to_datetime(df_YF['Date'])

df_CM.set_index('time', drop=True, inplace=True) 
df_YF.set_index('Date', drop=True, inplace=True) 
